...
function AddFreeServices({handleClose,handleOpen,open,data:temp,update}) {    
...
function handleSubmit(e){
  e.preventDefault();
  let url = update? '/admin/freeServices/update':'/admin/freeServices';
  Api.post(url, data)
  .then(() => {
      dispatch(fetchFreeServices())
      setData({service:""})
        ...
        <Box sx={style} className='model'>
          ...
          <form onSubmit={handleSubmit}>
            <Grid container spacing={2}>
                <Grid item xs={12}>
                    <InputLabel htmlFor="chaletImg">اسم خدمة مجانية</InputLabel>
...
export default AddFreeServices;
