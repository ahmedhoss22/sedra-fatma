...
import DeferredReservationsModal from '../modals/DeferredReservationsModal';
...
const DeferredReservations = () => {
  ...
  const data=useSelector((state)=>state.reservation.value.deferred)
  console.log(data);
  useEffect(()=>{dispatch(fetchReservations())},[])
  let filteredData=data
  if(search) filteredData=filteredData.filter((ele)=>ele.client.name.includes(search) || ele.date.includes(search) || ele.entity.name.includes(search) || ele.contractNumber>=search ||ele.date.includes(search)||ele.period.dayPeriod.includes(search)  )

  return (
    <div className="cont" style={{direction:i18n.language=='en'?'ltr':"rtl"}}>
    <h2>{t("reservation.deferred")}</h2>
